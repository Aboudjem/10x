...
- **humanizer** pinned at **0.6.2** (was 0.1.0, five minor versions behind its repo) and described with **55** AI writing patterns (was 43) in `marketplace.json`, `README.md`, the four translations, `llms.txt`, `site/index.html`, `AGENTS.md`, `ECOSYSTEM.md`, `QUALITY-BAR.md` and the title and aria-label of `humanizer-diagram.svg`.
- recap-studio's "one-page explainers in under 5 minutes" (10x's own wording) now says "you can read offline in about 5 minutes", which is what its `plugin.json` states; the `ATTRIBUTED` label in `aws-cost-audit-diagram.svg` moved below the ring it straddled and the overflowing `verified live · $/unit` tag reads `verified live`.
