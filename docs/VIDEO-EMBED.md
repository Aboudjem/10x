...
This is a **marketplace montage**: a single ~57s reel covering seven of the eight 10x plugins:
...
> `.sub` line, which still reads "7 plugins &middot; zero dependencies &middot; runs locally" and is now one
> short. It is **not** generated by
> `make-titles.sh`, so bump it to 8 by hand and rebuild `clips/c00.mp4` (4.0 s, 30 fps, keep that
