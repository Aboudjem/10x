...
This is a **marketplace montage**: a single ~57s reel covering all seven 10x plugins:
...
7. `loopify`: hands Claude a job that repeats, on an interval it keeps

> Note: `goalify`, `humanizer` and `loopify` are skill-only, so they have no standalone demo
> clip. Each appears as a title-card beat (`scenes/goalify.html` -> `clips/cg.mp4`, green `#3FB950`;
> `scenes/humanizer.html` -> `clips/chum.mp4`, magenta `#d946ef`; `scenes/loopify.html` ->
...
loopify title card (all three skill-only, no hero segment) → editors strip → close card with
the repo URL. The four hero segments are trimmed straight from each plugin repo's own
`demo-output/output.mp4`, so they are authentic.

> Heads-up when a plugin is added: `scenes/00-intro.html` hard-codes the plugin count in its
> `.sub` line ("7 plugins &middot; zero dependencies &middot; runs locally"). It is **not** generated by
> `make-titles.sh`, so bump it by hand and rebuild `clips/c00.mp4` (4.0 s, 30 fps — keep that
> duration or every `xfade.txt` offset shifts). Its terminal caret is an infinite blink, so
> each screenshot catches it in a random phase; both phases are correct.

The 14 clips concat in this order, with a 0.3 s crossfade at each seam (`xfade.txt`):
`c00` intro, `c01`+`p01` sniff, `c02`+`p02` ui-ux-suite, `c03`+`p03` recap-studio,
`c04`+`p04` aws-cost-audit, `cg` goalify, `chum` humanizer, `cl` loopify, `c05` editors,
`c06` close. The loopify card sits at 47.633 s; the editors and close beats follow at
50.333 s and 53.033 s. Each title card is a 3.0 s still (1920x1080, 30 fps, yuv420p),
built with `ffmpeg -loop 1 -i frames/<card>.png -t 3 -r 30`.
...
- The voiced `output.mp4` and its script `narration/vo.txt` are still the
  ...
  silent montage and the GIF carry all seven beats. Rewrite `vo.txt` before
  re-muxing, or the narration will not match the video.
...
ffmpeg -y -i demo-output/output_silent.mp4 -i demo-output/audio/vo.mp3 \
  -filter_complex "[1:a]adelay=1000|1000,afade=t=in:st=1:d=0.4,afade=t=out:st=52.1:d=2,apad,aresample=44100[a]" \
  -map 0:v -map "[a]" -c:v copy -c:a aac -b:a 160k -t 57.033 -movflags +faststart \
...
# if playwright has no browser downloaded ("Executable doesn't exist at ..."), either run
# `playwright install`, or screenshot with the Chrome already on the machine:
#   "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" --headless=new --disable-gpu \
#     --hide-scrollbars --force-device-scale-factor=1 --window-size=1920,1080 \
#     --virtual-time-budget=2500 --screenshot="$PWD/frames/<card>.png" "file://$PWD/scenes/<card>.html"
# the cards animate in over 0.9 s, so the wait/virtual-time budget matters
...
Current numbers for the seven-beat reel: `output_silent.mp4` is 57.033 s / 1711 frames at
